...
type StockInventoryLine struct {
	...
	Inventory           *StockInventory     `orm:"rel(fk)"`                              
	Location            *StockLocation      `orm:"rel(fk);null"`                         
	Product             *ProductProduct     `orm:"rel(fk);null"`                         
	Template            *ProductTemplate    `orm:"rel(fk);null"`                         
	Package             *StockQuantPackage  `orm:"rel(fk);null"`                         
	ProdLot             *StockProductionLot `orm:"rel(fk);null"`                         
	Company             *Company            `orm:"rel(fk);null"`                         
	State               string              `orm:"default(draft)" json:"State"`          
	ProductName         string              `orm:"default()" json:"ProductName"`         
	MeasureFirstUomQty  float64             `orm:"default(0)"`                           
	MeasureSecondUomQty float64             `orm:"default(0)"`                          
	CheckedFirstUomQty  float64             `orm:"default(0)"`                           
	CheckedSecondUomQty float64             `orm:"default(0)"`                           
