...
type StockMove struct {
	...
	Sequence           int64             `orm:"default(0)" json:"Sequence"`                
	Name               string            `json:"Name"`                                       
	Priority           int64             `orm:"default(1)" json:"Priority"`           
	Date               time.Time         `orm:" type(datetime)"`                             
	DateExpected       time.Time         `orm:" type(datetime)"`                 
	...
	ProductPackaging   *ProductPackaging `orm:"rel(fk);null"`                               
	LocationSrc        *StockLocation    `orm:"rel(fk);null"`                             
	LocationDest       *StockLocation    `orm:"rel(fk);null"`                                
	Partner            *Partner          `orm:"rel(fk);null"`                                
	Picking            *StockPicking     `orm:"rel(fk)"`                                     
	State              string            `orm:"default(draft)" json:"State"`               
	Note               string            `json:"Note"`                                       
	PartiallyAvailable bool              `orm:"default(false)"`                              
	PriceUnit          float64           `orm:"default(0)" json:"PriceUnit"`                 
	Company            *Company          `orm:"rel(fk)"`                                     
	BackOrder          *StockPicking     `orm:"rel(fk);null"`                           
	Origin             string            `json:"Origin"`                                    
	ProcureMethod      string            `orm:"default(make_to_order)" json:"ProcureMethod"` 
	Scrapped           bool              `orm:"default(false)" json:"Scrapped"`        
	Quants             []*StockQuant     `orm:"rel(m2m);rel_table(stock_quant_move_rel)"`    
	ReservedQuant      []*StockQuant     `orm:"reverse(many)"`                               
	Inventory          *StockInventory   `orm:"rel(fk);null"`                                
	WareHouse          *StockWarehouse   `orm:"rel(fk);null"`                         
