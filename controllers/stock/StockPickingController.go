...
func (ctl *StockPickingController) Get() {
	...
	switch direction {
	case "incoming":
		ctl.PageName = "Inbound order management"
		ctl.Data["MenuStockPickingIncomingActive"] = "active"
	case "outgoing":
		ctl.PageName = "Outbound order management"
		ctl.Data["MenuStockPickingOutgoingActive"] = "active"
	case "internal":
		ctl.PageName = "Transfer order management"
...
func (ctl *StockPickingController) Put() {
	...
	ctl.URL = "/stock/picking/"
	//Need to judge the page that does not need to jump when the file is uploaded
