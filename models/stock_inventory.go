...
type StockInventory struct {
	...
	Date       time.Time             `orm:"auto_now;type(datetime)" json:"Date"`  
	Lines      []*StockInventoryLine `orm:"reverse(many)"`                       
	Moves      []*StockMove          `orm:"reverse(many)"`                   
	...
	Lot        *StockProductionLot   `orm:"rel(fk);null"`                         
	Filter     string                `orm:"default(all)"`                         
