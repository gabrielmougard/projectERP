...
type PurchaseOrder struct {
	...
	PurchasesMan *User                `orm:"rel(fk)"`                             
	Company      *Company             `orm:"rel(fk)"`                              
	Country      *AddressCountry      `orm:"rel(fk);null" json:"country"`         
	Province     *AddressProvince     `orm:"rel(fk);null" json:"province"`        
	City         *AddressCity         `orm:"rel(fk);null" json:"city"`             
	District     *AddressDistrict     `orm:"rel(fk);null" json:"district"`         
	Street       string               `orm:"default()" json:"street"`          
	OrderLine    []*PurchaseOrderLine `orm:"reverse(many)"`                        
	State        *PurchaseOrderState  `orm:"rel(fk)"`                              
